...
def main():
    ...
    for split_name, split_df in [("train", train_df), ("dev", dev_df), ("test", test_df)]:
        ...
        # Save with metadata: taxonomy/quality + assembly provenance columns
        # (assembly level, isolate-vs-MAG, contiguity) so the selection is fully
        # auditable. Provenance columns are included only if present in the source.
        meta_file = output_dir / f"{split_name}_metadata.tsv"
        base_cols = ['accession', 'genus', 'family', 'order', 'phylum',
                     'gtdb_taxonomy', 'genome_size', 'checkm2_completeness',
                     'checkm2_contamination']
        provenance_cols = ['ncbi_assembly_level', 'ncbi_genome_category',
                           'contig_count', 'n50_contigs', 'longest_contig']
        meta_cols = base_cols + [c for c in provenance_cols if c in split_df.columns]
        split_df[meta_cols].to_csv(meta_file, sep='\t', index=False)
