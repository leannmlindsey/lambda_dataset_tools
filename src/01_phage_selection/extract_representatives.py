...
One genome per cluster is selected at random (seeded) as the representative,
to avoid biasing toward whatever ordering vclust uses within a cluster.
...
def main():
    ...
    # Select one representative per cluster at random (seeded for reproducibility).
    # Clusters are visited in sorted order so the RNG draw sequence is deterministic.
    representatives = []
    cluster_ids = []

    for cluster in sorted(cluster_to_genomes.keys()):
        genomes = cluster_to_genomes[cluster]
        representatives.append(random.choice(genomes))
